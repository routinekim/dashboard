...
- **디자인**: 다크 테마, 그라데이션 배경(딥 네이비 → 퍼플), 네온 포인트 컬러, 애니메이션 차트, 반응형 레이아웃
...
- 디자인: 다크 테마, 딥 네이비 → 퍼플 그라데이션, 네온 포인트 컬러

**시각 효과 표준 (모든 대시보드 컴포넌트에 적용):**

- **파티클 배경**: `useMemo`로 20개 입자 생성 → `floatParticle` 키프레임으로 위로 떠오르는 애니메이션, 색상은 보라/핑크/청록/민트/녹색 순환
- **배경 오브**: 위치·크기 다른 4개의 대형 블러 원(opacity 0.06)에 `orbFloat` 키프레임 적용
- **히어로 헤더**: `gradientShift` 키프레임으로 살아있는 그라데이션 텍스트 + `heroTextGlow` 발광 효과 + 스캔라인(`scanDown`)
- **스크롤 티커**: 핵심 성취 지표를 무한 좌우 스크롤 배너로 표시 (`ticker` 키프레임, 항목 2배 복사로 seamless loop)
- **카운트업 숫자**: `useEffect` + `requestAnimationFrame`으로 0→실제값 easeOut cubic 애니메이션 (duration 1600ms)
- **카드 스태거 입장**: 각 카드 `opacity 0→1`, `translateY 36px→0` 전환, `index × 85ms` delay로 순서 등장
- **네온 호버 글로우**: `onMouseEnter`에서 `box-shadow: 0 28px 64px {color}30, 0 0 0 1px {color}55` + `translateY(-8px) scale(1.03)` 적용
...
- **색상 규칙**: 1위(강점) `#4ade80`(녹), 최하위(약점) `#f87171`(적), 중위 `#fb923c`(주황), 특수 `#a78bfa`(보라)
